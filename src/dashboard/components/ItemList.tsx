...
export function ItemList({
  ...
  const formatDate = (timestamp: number): string => {
    ...
    if (days === 0) return lang === 'pt' ? 'Hoje' : lang === 'es' ? 'Hoy' : 'Today'
    if (days === 1) return lang === 'pt' ? 'Ontem' : lang === 'es' ? 'Ayer' : 'Yesterday'
    if (days < 7) {
      if (lang === 'pt') return `${days} dias atrás`
              ...
              >
                ...
                <div className="flex items-center gap-3">
                  ...
                  <Button
                    ...
                    onClick={(e) => {
                      e.stopPropagation()
                      if (window.confirm(t('dashboard.trash.confirmDelete'))) {
                        onDelete(item.id)
                      }
                    }}
